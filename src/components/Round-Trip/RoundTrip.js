...
import { IoSwapHorizontal } from "react-icons/io5";
  ...
  const [saveSearch, setSaveData] = useState();
  ...
  function SearchSaved() {
    if (tripType === "round") {
      ...
      const roundTripData = {
        tripType: "RoundTrip",
        fromWhere: fromTypedInput.label,
        toWhere: whereTypedInput.label,
        // departureDate: saveSearch.departureDate,
        returnDate: saveSearch.returnDate,
        traveler: `${numTravelers} Traveler(s)`,
      };

      setStoreData([...storeData, roundTripData]);
      console.log("Round Trip Data Saved:", roundTripData);
      ...
      setStoreData([...storeData, oneWayData]);
      console.log("One Way Data Saved:", oneWayData);
    }
  }
  
  const token = "U4h5LGAoH7vN2RoVHJQlGvcIamu3";
  
  // Function to swap the 'from' and 'where' locations
  const swapLocations = () => {
    setFromTypedInput(whereTypedInput);
    setWhereTypedInput(fromTypedInput);
  };
      ...
      <div className="search__container">
       ...
       <div onClick={()=>setTripType("one")} className="radio-btn"><Radio2 tripType={tripType}/><span>One Way</span></div>
        ...
        <div className="search__sub_container">
          ...
          {/*INSERT SWAP LOGO HERE*/}
          <div className="swap__button" onClick={swapLocations}>
               <IoSwapHorizontal />
          </div>
